...
const generateSimulatedData = (token: string, timeframe: string): CombinedTokenData => {
  ...
  // Generate random starting values based on token
  const tokenSeed = token.charCodeAt(0) + token.charCodeAt(token.length - 1);
  const randomBase = (tokenSeed % 10) / 10;
  
  // Basic price and metrics - these would come from CoinGecko in real app
  const currentPrice = 1000 + (randomBase * 9000);
  const yesterdayPrice = currentPrice * (0.9 + (Math.random() * 0.2));
  const priceChange24h = calculatePercentChange(yesterdayPrice, currentPrice);
  
  const currentVolume = 1000000 + (randomBase * 9000000);
  const yesterdayVolume = currentVolume * (0.85 + (Math.random() * 0.3));
  const volumeChange24h = calculatePercentChange(yesterdayVolume, currentVolume);
  
  const currentAddresses = 1000 + (randomBase * 4000);
  const yesterdayAddresses = currentAddresses * (0.9 + (Math.random() * 0.2));
  ...
  let simulatedActivity = 10 + (randomBase * 20);
  let simulatedSentiment = -20 + (randomBase * 80);
  ...
  // Generate a reasonable sentiment value between -100 and 100
  const overallSentiment = -50 + (randomBase * 100);
  ...
  // Twitter sentiment is typically 20% more volatile than overall
  // Reddit sentiment is typically 30% more volatile than overall
  // These are approximations for demonstration
  const twitterSentiment = overallSentiment * 1.2;
  const redditSentiment = overallSentiment * 1.3;
  const newsSentiment = overallSentiment * 0.8;
  
  const sentData: SentimentData = {
    overall: overallSentiment,
    social: (twitterSentiment + redditSentiment) / 2,
    news: newsSentiment,
    positiveTopics,
    negativeTopics,
    sentimentHistory,
