...
async function runChatMode(agent: any, config: any) {
  // If a prompt is provided as a command-line argument, run single prompt mode
  const promptArg = process.argv[2];
  if (promptArg) {
    await runSinglePrompt(agent, config, promptArg);
    return;
  }
...
async function runSinglePrompt(agent: any, config: any, prompt: string) {
  ...
  // Print only the response for backend integration
